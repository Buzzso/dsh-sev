...
// ── routes ──────────────────────────────────────────────────────────────────
/** Registry-global set of archived remote session ids (kept in sync by the
 *  archive route; sessions whose id is here are hidden from the list). */
const archivedIds = new Set()

function makeRoutes({ registry, allocatePort, getTunnel = tunnelStatus, markRunning = markTunnelRunning, getArchived = () => archivedIds }) {
    ...
    },
    {
      ...
      handler: withFence(async (req, res) => {
        ...
        try {
          ...
          archivedIds.clear()
          for (const sid of data.result.value?.archivedSessionIds ?? []) archivedIds.add(sid)
          return writeJson(res, 200, { ok: true, archivedSessionIds: [...archivedIds] })
        } catch (error) {
          return writeJson(res, 502, { error: String(error) })
        }
      }),
